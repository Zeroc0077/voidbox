...
pub async fn save(db: &D1Database, entry: &MailEntry, received_at: u64, headers_json: &str) -> Result<()> {
    db.prepare(
        "INSERT INTO mail (id, inbox_id, from_addr, forward_from, subject, text_body, html_body, headers, received_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, ?9)"
    )
        .bind(&[
            entry.id.as_str().into(),
            entry.inbox.as_str().into(),
            entry.from.as_str().into(),
            entry.forward_from.as_str().into(),
...
pub async fn find_all(db: &D1Database, inbox_id: &str) -> Result<Vec<MailRow>> {
    let results = db.prepare(
        "SELECT id, from_addr, forward_from, subject, received_at FROM mail WHERE inbox_id = ?1 ORDER BY received_at DESC"
    )
        .bind(&[inbox_id.into()])?
        .all()
        .await?;
...
pub async fn find_by_sender(db: &D1Database, inbox_id: &str, sender: &str) -> Result<Vec<MailRow>> {
    let results = db.prepare(
        "SELECT id, from_addr, forward_from, subject, received_at FROM mail WHERE inbox_id = ?1 AND LOWER(from_addr) = LOWER(?2) ORDER BY received_at DESC"
    )
...
/// Fetch email summaries filtered by exact forwarding source (case-insensitive).
pub async fn find_by_forwarder(db: &D1Database, inbox_id: &str, forwarder: &str) -> Result<Vec<MailRow>> {
    let results = db.prepare(
        "SELECT id, from_addr, forward_from, subject, received_at FROM mail WHERE inbox_id = ?1 AND LOWER(forward_from) = LOWER(?2) ORDER BY received_at DESC"
    )
        .bind(&[inbox_id.into(), forwarder.into()])?
        .all()
        .await?;
    results.results::<MailRow>()
}
...
pub async fn find_one(db: &D1Database, inbox_id: &str, mail_id: &str) -> Result<Option<FullMailRow>> {
    db.prepare(
        "SELECT id, inbox_id, from_addr, forward_from, subject, text_body, html_body, headers, received_at FROM mail WHERE id = ?1 AND inbox_id = ?2"
    )
